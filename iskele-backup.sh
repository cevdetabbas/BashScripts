...
│    🗑️ $(basename "$oldest") ($(human_bytes "$osize"))"
...
if [ "$TAR_SIZE_BYTES" -gt "$LIMIT_BYTES" ]; then
  CLEANUP_STATUS="❌ LOCAL FILE TOO BIG (${TAR_SIZE_H}) – SCP SKIPPED"
  ...
  if [ "$STATUS" = "NO_MORE_DAILY" ]; then
    CLEANUP_STATUS="❌ REMOTE FULL (only Weekly/Monthly left) – SCP SKIPPED"
    CLEANUP_REASON="remote_only_protected"
  else
    if [ "$DELETED_COUNT" -gt 0 ]; then
      CLEANUP_STATUS="✅ REMOTE CLEANUP (daily): ${DELETED_COUNT} file(s)"
    else
      CLEANUP_STATUS="✅ NO REMOTE CLEANUP NEEDED"
    fi
    CLEANUP_REASON="ok"
  fi
fi
...
echo "" >> "$LOGFILE"
echo "╭─ ${HEADER} ─────────────────────────────────────────────" >> "$LOGFILE"
echo "│ 📂 Source ..................... /home/tm (tar only)" >> "$LOGFILE"
echo "│ 📁 Tar Size ................... $TAR_SIZE_H" >> "$LOGFILE"
echo "│ 🌐 Remote Folder (pre) ........ $PRE_REMOTE_H" >> "$LOGFILE"
echo "│ 🌐 Remote Folder (post) ....... $POST_REMOTE_H" >> "$LOGFILE"
echo "│ 📦 Max Remote Folder Size ..... $LIMIT_H" >> "$LOGFILE"
echo "│ 🧰 Tar Time ................... $TAR_TIME_H" >> "$LOGFILE"
echo "│ 🚀 SCP Transfer Time .......... $SCP_TIME_H" >> "$LOGFILE"
echo "│ ⏱️ Total Time ................. $TOTAL_TIME_H" >> "$LOGFILE"
echo "│ 🧹 Cleanup .................... $CLEANUP_STATUS" >> "$LOGFILE"

if [ "$DELETED_COUNT" -gt 0 ]; then
  echo "│   Deleted daily files:" >> "$LOGFILE"
  echo "$CLEANUP_DETAILS" >> "$LOGFILE"
fi

if [ "$SCP_EXIT" -eq 0 ]; then
  echo "│ ✅ SCP ........................ SUCCESS" >> "$LOGFILE"
elif [[ "$CLEANUP_STATUS" == ❌* ]]; then
  echo "│ ✅ SCP ........................ SKIPPED" >> "$LOGFILE"
else
  echo "│ ✅ SCP ........................ FAILED (after retries)" >> "$LOGFILE"
fi

echo "│ 🗓️ Weekly Copy ................ $WEEKLY_ACTION" >> "$LOGFILE"
echo "│ 📅 Monthly Copy ............... $MONTHLY_ACTION" >> "$LOGFILE"

echo "│" >> "$LOGFILE"
echo "│ ✅ Destination: ${DEST_HUMAN}" >> "$LOGFILE"
echo "╰──────────────────────────────────────────────────────────" >> "$LOGFILE"
echo "" >> "$LOGFILE"

echo "✅ Backup tamamlandı! Log: $LOGFILE"
